...
im = double(imread ('phpaQcVXp.png'));
[H,W] = size(im);
im = im + randn(H,W)*5;
im2 = im; im2(:,:) = 0;
...
P = 6; % radius of window to compute mean, so window will be 23 x 23
sig1 = 35; sig2 = 35; % kernel parameters for xy nd itnensity respectively
numiters = zeros(H,W); % array to record number of iterations to reach convergence
cells = cell(H,W);
...
for i=1:H
    ...
    for j=1:W
        ...
        while (1)
            ...
            if sum((curr-oldcurr).^2) < epsilon, break; end 
            oldcurr = curr;
        end
